...
print(f_list)
i=1
t_list=as.numeric(substr(strsplit(f_list[i], "_")[[1]][4],2,nchar(strsplit(f_list[i], "_")[[1]][4])))
...
plot(main = "M|P Block 5 prime head to nearest dREG sites (Both up and downstream)",
     data$V1[data$V1 <= 20000],  (data$V2/data$V2[length(data$V2)])[data$V1 <= 20000], type='l', pch=20,
...
for (i in 2:length(f_list)){
  #print(f_list[i])
  data = read.table(f_list[i])
  t_list=c(t_list, as.numeric(substr(strsplit(f_list[i], "_")[[1]][4],2,nchar(strsplit(f_list[i], "_")[[1]][4]))))
