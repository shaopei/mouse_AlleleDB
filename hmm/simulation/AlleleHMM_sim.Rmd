...
Nevertheless, a survey spanning all of the hundreds of mammalian genes characterized to date would seem to suggest that mdx is an extreme example, with most genes falling into the range of 10-80 kb, with an median size in the range of 20 kb. If every SNPs 140bp (129/castaneus F1 hybrid mES cells), then 142 SNPs per gene.
...
wrap codes into a function to test sensitivity  (Use this one for Binomial test)
```{r}
...
Sensitivity_iter<- function(iteration,l, e, mat_p, t=1e-5){
  SNP_sen_list=c()
  HMM_sen_list=c()
  SNP_spec_list=c()
  HMM_spec_list=c()
...
atest=mclapply(seq(0.1,0.9,0.05),FUN=function(idx){m_test(idx,10)}, mc.cores = 1)

m=Get_table_from_simulation(atest)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "MatP_SMS_OD0_l10_sensitivity_and_specificity_iter1K.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("MatP_SMS_OD0_l10_sensitivity_iter1k.pdf")
ggplot(m, aes(x=MatP, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() +    
   scale_color_manual(values=c("red", "blue")) +
    ...
    #theme_bw()
dev.off()
pdf("MatP_SMS_OD0_l10_specificity_iter1k.pdf")
ggplot(m, aes(x=MatP, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point()+    
...
e_test_out=mclapply(seq(1,50,1),FUN=function(idx){e_test(idx,10)}, mc.cores = 1)


result=unlist(e_test_out)
...
m1=data.frame(result[seq(1,length(result),9)],
              ...
              result[seq(6,length(result),9)],
              result[seq(8,length(result),9)])
names(m1)=c("exp","senMean", "senSE", "specMean", "specSE")
m1$method="SNP independantly"
#View(m)
m2=data.frame(result[seq(1,length(result),9)],
              result[seq(3,length(result),9)], 
              ...
              result[seq(7,length(result),9)],
              result[seq(9,length(result),9)])
names(m2)=c("exp","senMean", "senSE","specMean", "specSE")
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "exp_SMS_OD0_l20_sensitivity_and_specificity_iter1K.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("exp_SMS_OD0_l20_sensitivity_iter1K.pdf")
ggplot(m, aes(x=exp, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() +    
   scale_color_manual(values=c("red", "blue")) +
    ...
    #theme_bw()
dev.off()
pdf("exp_SMS_OD0_l20_specificity_iter1K.pdf")
ggplot(m, aes(x=exp, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point()+    
   scale_color_manual(values=c("red", "blue")) +
...
#l_test_out=mclapply(c(seq(1,50,1)),FUN=function(idx){l_test(idx,1000)}, mc.cores = 50)
l_test_out=mclapply(seq(1,20,2),FUN=function(idx){l_test(idx,10)}, mc.cores = 1)
...
#result_to_ggplot(l_test_out,"length_and_specificity_e5_OD0.txt","length", "length_and_specificity_e5_OD0.pdf","Specificity")
#result_to_ggplot_both(l_test_out,"length_SSS.txt","length_SSS") 

result=unlist(l_test_out)
...
m1=data.frame(result[seq(1,length(result),9)],
              ...
              result[seq(6,length(result),9)],
              result[seq(8,length(result),9)])
names(m1)=c("length","senMean", "senSE", "specMean", "specSE")
m1$method="SNP independantly"
#View(m)
m2=data.frame(result[seq(1,length(result),9)],
              result[seq(3,length(result),9)], 
              ...
              result[seq(7,length(result),9)],
              result[seq(9,length(result),9)])
names(m2)=c("length","senMean", "senSE","specMean", "specSE")
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "length_SSS_OD0_e5_sensitivity_and_specificity_iter1K.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("length_SSS_OD0_e5_sensitivity_iter1K.pdf")
ggplot(m, aes(x=length, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point()+
    scale_color_manual(values=c("red", "blue")) +
    ...
    #theme_bw()
dev.off()
pdf("length_SSS_OD0_e5_specificity_iter1K.pdf")
ggplot(m, aes(x=length, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point() +
    scale_color_manual(values=c("red", "blue")) +
...
library(parallel)
e_test_out=mclapply(seq(1,10,1),FUN=function(idx){e_test(idx,10)}, mc.cores = 1)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
#write.table(m, file = "exp_SSS_OD0_l10_sensitivity_and_specificity_iter1K.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
#pdf("exp_SSS_OD0_l10_sensitivity_iter1K.pdf")
ggplot(m, aes(x=exp, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() +    
   scale_color_manual(values=c("red", "blue")) +
    ...
    #theme_bw()
#dev.off()
#pdf("exp_SSS_OD0_l10_specificity_iter1K.pdf")
ggplot(m, aes(x=exp, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point()+    
...
```{r}
setwd("~/Box Sync/Danko_lab_work/mouse_AlleleDB/hmm/simulation")
m=read.table("binomial/SSS_blocks/expressionLevel_SSS.txt", header=T,sep = "\t")
...
mplot + theme(text = element_text(size=25))
#output 8x5
...
m=read.table("exp_and_sensitivity_iter1000.txt", header=T,sep = "\t")
  ...
  m=read.table("binomial/SMS/length_SMS_OD0_e5_sensitivity_and_specificity_iter1K.txt", header=T,sep = "\t")
  mplot<- ggplot(m, aes(x=length, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.1) +
    geom_line() +
    geom_point()+
    ...
    ylab("Sensitivity")+  
    ylim(c(0,1))+
    xlim(c(0,40))+ scale_color_manual(values=c("red", "blue"))
  #theme_bw() 
  mplot+theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))

  
```

Use Beta-Binomial, instead of Binomial

Mu = shape1 / (shape1+shape2)
Rho = 1/(1+shape1+shape2)
Binomial Rho=0, shape1 and shape2 --> inf

```{r}
#From Allele_seq_analysis_20170613.R
#GRO-seq data of GM12878
s1_all = 1.571751 
s2_all = 1.579117
Mu_all=s1_all/(s1_all+s2_all) #0.4988311
Rho_all=1/(1+s1_all+s2_all) # 0.2409135
s1_Asym = 0.3774592 
s2_Asym = 0.3816091
Rho_Asym=1/(1+s1_Asym+s2_Asym) #0.5684828
s1_Sym = 28.51298 
s2_Sym = 28.54951
Rho_Sym =1/(1+s1_Sym+s2_Sym) #0.01722282
```
...
test MatP in betaBinomial with differet OD
```{r}
## test multi-threads with MatP
...
m_test <- function(mat_p_2, iteration){
  ...
  mat_p=c(0.5,mat_p_2,0.5)
  #print(mat_p)
  #aveS = Sensitivity_betaBino_iter(iteration,l, e, mat_p, od)
  aveS = Precision_recall_specificity_betaBino_iter(iteration,l, e, mat_p, od, t)
  return(c(mat_p_2,aveS))
}


od<-c(0,0.01,0)
test_0.01=mclapply(seq(0,1,0.05),FUN=function(idx){m_test(idx,iteration = 100)}, mc.cores = 10)
m1=Get_table_from_simulation(test_0.01)
m1$OD=0.01

od<-c(0,0.1,0)
test_0.1=mclapply(seq(0,1,0.05),FUN=function(idx){m_test(idx,iteration = 100)}, mc.cores = 10)
m2=Get_table_from_simulation(test_0.1)
m2$OD=0.1

od<-c(0,0.18,0)
test_0.18=mclapply(seq(0,1,0.05),FUN=function(idx){m_test(idx,iteration = 100)}, mc.cores = 10)
m5=Get_table_from_simulation(test_0.18)
m5$OD=0.18

od<-c(0,0.25,0)
test_0.25=mclapply(seq(0,1,0.05),FUN=function(idx){m_test(idx,iteration = 100)}, mc.cores = 50)
m3=Get_table_from_simulation(test_0.25)
m3$OD=0.25

od<-c(0.25,0.25,0.25)
test_3.25=mclapply(seq(0,1,0.05),FUN=function(idx){m_test(idx,iteration = 100)}, mc.cores = 50)
m4=Get_table_from_simulation(test_0.25)
m4$OD=0.25


m=rbind.data.frame(m1,m2, m5,m3,m4)
write.table(m, file = "matP_SMS_ODx_sensitivity_and_precision_iter100.txt", quote = F, sep = "\t",row.names = F)
...
pdf("matP_SMS_ODx_precision.pdf")
method="Allele HMM"
plot(m1$trait[m1$method==method], m1$precMean[m1$method==method], col=red_cols[2],ylab="mean of precision", xlab="expected maternal reads fraction")
lines(m1$trait[m1$method==method], m1$precMean[m1$method==method], col=red_cols[2])
lines(m2$trait[m2$method==method], m2$precMean[m2$method==method], col=red_cols[4])
points(m2$trait[m2$method==method], m2$precMean[m2$method==method], col=red_cols[4])
lines(m5$trait[m2$method==method], m5$precMean[m5$method==method], col=red_cols[5])
points(m5$trait[m2$method==method], m5$precMean[m5$method==method], col=red_cols[5])
points(m3$trait[m3$method==method], m3$precMean[m3$method==method], col=red_cols[6])
lines(m4$trait[m4$method==method], m4$precMean[m4$method==method], col=red_cols[7])
#points(m4$trait[m4$method==method], m4$precMean[m4$method==method], col="blue")

method="SNP independantly"
points(m1$trait[m1$method==method], m1$precMean[m1$method==method], col=blue_cols[2])
lines(m1$trait[m1$method==method], m1$precMean[m1$method==method], col=blue_cols[2])
lines(m2$trait[m2$method==method], m2$precMean[m2$method==method], col=blue_cols[4])
points(m2$trait[m2$method==method], m2$precMean[m2$method==method], col=blue_cols[4])
lines(m5$trait[m2$method==method], m5$precMean[m5$method==method], col=blue_cols[5])
points(m5$trait[m2$method==method], m5$precMean[m5$method==method], col=blue_cols[5])
points(m3$trait[m3$method==method], m3$precMean[m3$method==method], col=blue_cols[6])
lines(m4$trait[m4$method==method], m4$precMean[m4$method==method], col=blue_cols[7])

legend("bottomleft", lty =1,col=c(red_cols[7], rev(red_cols[4:5]), red_cols[2]),pch=1,
       legend=c("0.25", "0.18", "0.10", "0.01"), cex=1.5,lwd=2, bty="n", title = "Allele HMM OD")
legend("bottomright", lty =1,col=c(blue_cols[7], rev(blue_cols[4:5]), blue_cols[2]),pch=1,
       legend=c("0.25", "0.18", "0.10", "0.01"), cex=1.5,lwd=2, bty="n", title ="SNP independantly OD")
dev.off()


pdf("matP_SMS_ODx_sensitivity.pdf")
method="Allele HMM"
plot(m1$trait[m1$method==method], m1$senMean[m1$method==method], col=red_cols[2],ylab="mean of sensitivity", xlab="expected maternal reads fraction")
lines(m1$trait[m1$method==method], m1$senMean[m1$method==method], col=red_cols[2])
lines(m2$trait[m2$method==method], m2$senMean[m2$method==method], col=red_cols[4])
points(m2$trait[m2$method==method], m2$senMean[m2$method==method], col=red_cols[4])
lines(m5$trait[m2$method==method], m5$senMean[m5$method==method], col=red_cols[5])
points(m5$trait[m2$method==method], m5$senMean[m5$method==method], col=red_cols[5])
points(m3$trait[m3$method==method], m3$senMean[m3$method==method], col=red_cols[6])
lines(m4$trait[m4$method==method], m4$senMean[m4$method==method], col=red_cols[7])
#points(m4$trait[m4$method==method], m4$senMean[m4$method==method], col="blue")

method="SNP independantly"
points(m1$trait[m1$method==method], m1$senMean[m1$method==method], col=blue_cols[2])
lines(m1$trait[m1$method==method], m1$senMean[m1$method==method], col=blue_cols[2])
lines(m2$trait[m2$method==method], m2$senMean[m2$method==method], col=blue_cols[4])
points(m2$trait[m2$method==method], m2$senMean[m2$method==method], col=blue_cols[4])
lines(m5$trait[m2$method==method], m5$senMean[m5$method==method], col=blue_cols[5])
points(m5$trait[m2$method==method], m5$senMean[m5$method==method], col=blue_cols[5])
points(m3$trait[m3$method==method], m3$senMean[m3$method==method], col=blue_cols[6])
lines(m4$trait[m4$method==method], m4$senMean[m4$method==method], col=blue_cols[7])
...
library(ggplot2)
pdf("matP_SMS_ODx_sensitivity.pdf")
ggplot(m, aes(x=trait, y=senMean, colour=OD)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() +
  xlab("matP")
...
test multi-threads with MatP in betaBinomial
```{r}
## test multi-threads with MatP
...
#test=mclapply(seq(0,1,0.05),FUN=function(idx){m_test(idx,iteration = 1000)}, mc.cores = 50)
test=mclapply(seq(0,1,0.05),FUN=function(idx){m_test(idx,iteration = 10)}, mc.cores = 1)
result=unlist(test)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "matP_SMS_OD0.25_sensitivity_and_specificity_iter1K.txt", quote = F, sep = "\t",row.names = F)

#m=read.table("BetaBinomial/Sensitivity==M/od0.25/matP_SMS_OD0.25_sensitivity_and_specificity_iter1K.txt", sep = "\t",header = T)

library(ggplot2)
pdf("matP_SMS_OD0.5_sensitivity.pdf")
ggplot(m, aes(x=matP, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() #+ 
    #theme_bw()
dev.off()
pdf("matP_SMS_OD0.5_specificity.pdf")
ggplot(m, aes(x=matP, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point() #+ 
...
library(parallel)
l_test_out=mclapply(seq(1,50,10),FUN=function(idx){l_test(idx,3)}, mc.cores = 1)
l_test_out=mclapply(seq(1,50,1),FUN=function(idx){l_test(idx,1000)}, mc.cores = 20)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "length_SMS_OD0.25_sensitivity_and_specificity_iter1K.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("length_SMS_OD0.25_sensitivity_iter1K.pdf")
ggplot(m, aes(x=length, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() #+ 
    #theme_bw()
dev.off()
pdf("length_SMS_OD0.25_specificity.pdf")
ggplot(m, aes(x=length, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point() #+ 
...
e_test <- function(e_2, iteration){
  e=c(10, e_2, 10)
  aveS = Sensitivity_betaBino_iter(iteration,l, e, mat_p, od, t=1e-5) 
  return(c(e_2,aveS))
}
e_test_out=mclapply(seq(1,50,1),FUN=function(idx){e_test(idx,1000)}, mc.cores = 50)
e_test_out=mclapply(seq(1,500,10),FUN=function(idx){e_test(idx,10)}, mc.cores = 20)
e_test_out=mclapply(seq(1,150,1),FUN=function(idx){e_test(idx,1000)}, mc.cores = 50)
e_test_out=mclapply(seq(1,500,50),FUN=function(idx){e_test(idx,3)}, mc.cores = 1)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "exp_SMS_OD0.25_sensitivity_and_specificity_iter1K.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("exp_SMS_OD0.25_sensitivity_t1e-5_iter1K.pdf")
ggplot(m, aes(x=exp, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() #+ 
    #theme_bw()
dev.off()
pdf("exp_SMS_OD0.25_specificity_t1e-5_iter1K.pdf")
ggplot(m, aes(x=exp, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point() #+ 
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "length_SSS_OD0.25_sensitivity_and_specificity_iter1K.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("length_SSS_OD0.25_sensitivity_iter1K.pdf")
ggplot(m, aes(x=length, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point()+    
   scale_color_manual(values=c("red", "blue")) +
    ...
    #theme_bw()
dev.off()
pdf("length_SSS_OD0.25_specificity_iter1K.pdf")
ggplot(m, aes(x=length, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point() +    
...
e_test_out=mclapply(seq(1,150,10),FUN=function(idx){e_test(idx,1000)}, mc.cores = 20)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "exp_SSS_OD0.25_sensitivity_and_specificity_iter1K_50.txt", quote = F, sep = "\t",row.names = F)

write.table(m, file = "exp_SSS_OD0.25_sensitivity_and_specificity_iter1K_150.10.txt", quote = F, sep = "\t",row.names = F)


library(ggplot2)
pdf("exp_SSS_OD0.25_sensitivity_t1e-5_iter100.pdf")
ggplot(m, aes(x=exp, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() #+ 
    #theme_bw()
dev.off()
pdf("exp_SSS_OD0.25_specificity_t1e-5_iter100.pdf")
ggplot(m, aes(x=exp, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point() #+ 
...
test multi-threads with overdispersion in betaBinomial
```{r}
## test multi-threads with MatP
...
test=mclapply(seq(0.01,0.5,0.01),FUN=function(idx){od_test(idx,iteration = 3)}, mc.cores = 1)
#test=mclapply(seq(0,0.9,0.05),FUN=function(idx){od_test(idx,iteration = 1000)}, mc.cores = 50)
...
m1=data.frame(result[seq(1,length(result),9)],
              ...
              result[seq(6,length(result),9)],
              result[seq(8,length(result),9)])
names(m1)=c("OD","senMean", "senSE", "specMean", "specSE")
m1$method="SNP independantly"
#View(m)
m2=data.frame(result[seq(1,length(result),9)],
              result[seq(3,length(result),9)], 
              ...
              result[seq(7,length(result),9)],
              result[seq(9,length(result),9)])
names(m2)=c("OD","senMean", "senSE","specMean", "specSE")
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "OD_SMS_and_sensitivity_and_specificity.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("OD_and_sensitivity.pdf")
ggplot(m, aes(x=OD, y=senMean, colour=method)) + 
    geom_errorbar(aes(ymin=senMean-senSE, ymax=senMean+senSE), width=.01) +
    geom_line() +
    geom_point() #+ 
    #theme_bw()
dev.off()
pdf("OD_and_specificity.pdf")
ggplot(m, aes(x=OD, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point() #+ 
    #theme_bw()
...
# specificity of SSS block
mat_p <- c(0.5,0.5,0.5)
SSS_test=mclapply(seq(0.01,0.5,0.01),FUN=function(idx){od_test(idx,iteration = 3)}, mc.cores = 1)
#SSS_test=mclapply(seq(0,0.9,0.05),FUN=function(idx){od_test(idx,iteration = 1000)}, mc.cores = 50)
result=unlist(SSS_test)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "OD_SSS_sensitivity_and_specificity.txt", quote = F, sep = "\t",row.names = F)
pdf("OD_SSS_specificity.pdf")
ggplot(m, aes(x=OD, y=specMean, colour=method)) + 
    geom_errorbar(aes(ymin=specMean-specSE, ymax=specMean+specSE), width=.01) +
    geom_line() +
    geom_point() #+ 
    #theme_bw()
dev.off()
#hist(rbetabinom(100, 10, 0.9, 0.5))

```


Change sensitivity calculation from =!S to ==M
