...
wrap codes into a function to test sensitivity  (Use this one)
```{r}
...
Sensitivity_betaBino<- function(l, e, mat_p, od){
  # Simulation of 3 HMM blocks (Sym-Mat-Sym)
  i=1
 ...
 blockList$AlleleDB_state[blockList$AlleleDB_p_value <0.05 & blockList$SimState=="M" & blockList$mat_reads/blockList$total_reads >= 0.5] <- "M"
 blockList$AlleleDB_state[blockList$AlleleDB_p_value <0.05 & blockList$SimState=="M" & blockList$mat_reads/blockList$total_reads < 0.5] <- "P" 
...
Sensitivity_betaBino_iter<- function(iteration,l, e, mat_p, od){
  SNP_sen_list=c()
  HMM_sen_list=c()
  SNP_spec_list=c()
  HMM_spec_list=c()
  for (i in 1:iteration) {
    #print (i)
    s=Sensitivity_betaBino(l, e, mat_p, od)
    SNP_sen_list=c(SNP_sen_list, s$SNP_Sensitivity)
    HMM_sen_list=c(HMM_sen_list, s$AlleleHMM_Sensitivity)
    SNP_spec_list=c(SNP_spec_list, s$SNP_Specificity)
    HMM_spec_list=c(HMM_spec_list, s$AlleleHMM_Specificity)
...
test=mclapply(seq(0,1,0.05),FUN=function(idx){m_test(idx,iteration = 1000)}, mc.cores = 10)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "matP_SMS_OD0.25_sensitivity_and_specificity.txt", quote = F, sep = "\t",row.names = F)
...
l_test_out=mclapply(seq(1,50,1),FUN=function(idx){l_test(idx,1000)}, mc.cores = 30)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "length_SMS_OD0.25_sensitivity_and_specificity.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("length_SMS_OD0.25_sensitivity.pdf")
...
e_test_out=mclapply(seq(1,150,1),FUN=function(idx){e_test(idx,1000)}, mc.cores = 30)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "exp_SMS_OD0.25_sensitivity_and_specificity.txt", quote = F, sep = "\t",row.names = F)

library(ggplot2)
pdf("exp_SMS_OD0.25_sensitivity.pdf")
ggplot(m, aes(x=exp, y=senMean, colour=method)) + 
    ...
    #theme_bw()
dev.off()
pdf("exp_SMS_OD0.25_specificity.pdf")
...
#test=mclapply(seq(0,0.9,0.1),FUN=function(idx){od_test(idx,iteration = 1000)}, mc.cores = 50)
...
m2$method="Allele HMM"
m=rbind.data.frame(m1,m2)
write.table(m, file = "OD_and_sensitivity_and_specificity.txt", quote = F, sep = "\t",row.names = F)
...
#SSS_test=mclapply(seq(0,0.9,0.1),FUN=function(idx){od_test(idx,iteration = 1000)}, mc.cores = 50)
