...
library("colorspace")
setwd("/Users/shaopei/Box\ Sync/Danko_lab_work/mouse_AlleleDB/GM12878/TssInBlocks/SRR4041366_dedup_2/AlleleHMM")
...
for (i in 2:length(f_list)){
  ...
  lines(data$V1,  (data$V2/data$V2[length(data$V2)]),lty=i, pch=20, lwd=2, col=rainbow_hcl(length(f_list))[i])
}
legend("bottomright", pch=NA, lty = rev(1:9),col=rev(rainbow_hcl(length(f_list))),
       legend=rev(formatC(t_list, format = "e", digits = 0)), cex=1.5,lwd=2, bty="n")
...
plot(log10(t_list), fraction_at5K, xlab='log10(t)', ylab='fraction of M|P blocks at 5Kb', 
     type="b", bty="n", las=1, bty="l", lwd=2)
plot(log10(t_list), fraction_at5K, xlab='log10(t)', ylab='fraction of M|P blocks at 5Kb', 
     type="b", bty="n", las=1, bty="l", lwd=2)
