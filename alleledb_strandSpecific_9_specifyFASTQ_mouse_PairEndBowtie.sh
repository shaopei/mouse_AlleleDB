...
if [[ ${FLAG} -eq 1 || ${FLAG} -eq 0 ]] ; then
	...
	zcat ${FASTQ_PATH}/${FASTQ}_R1.fastq.gz | python ${PL}/fastq2result.py - - |  python ${PL}/filter_query.py - tmp_R1.fasta
	zcat ${FASTQ_PATH}/${FASTQ}_R2.fastq.gz | python ${PL}/fastq2result.py - - |  python ${PL}/filter_query.py - tmp_R2.fasta
	
	#pair up R1 and R2
	python ${PL}/pair_fasta_R1_R2.py tmp
